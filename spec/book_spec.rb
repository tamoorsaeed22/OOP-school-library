...
describe Book do
  before :each do
    @book1 = Book.new('lard of the rings', 'martin')
    @book2 = Book.new('game of throne', 'george')
    @person = Person.new(98, name: 'Rosy')
  end

  context 'When creating a new book' do
    it 'should have the following attributes (title, author, and rentals)' do
      expect(@book1.title).to eql 'lard of the rings'
      expect(@book1.author).to eql 'martin'
